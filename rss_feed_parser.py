...
class RSSFeedParser:
    ...
    @timeit
    def _parse_and_analzye_single_rss_feed(self, rss_feed, dump_json=False, datetime_filter=None, known_articles_ids=[]):
        df_feed = pd.DataFrame(columns=["timestamp", "datetime", "title", "summary", "source",\
                                         "id", "keyword", "sentiment_score", "sentiment_label"])
        feed = feedparser.parse(rss_feed)
        if dump_json == True:
            with open(f"{rss_feed.split('.')[1]}.json", 'w') as fp:
                json.dump(feed, fp)
    ...
    def parse_and_analyze_rss_feeds(self, dump_json=False, save_to_db=True, datetime_filter=None, exclude_known_articles=True):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [executor.submit(self._parse_and_analzye_single_rss_feed, rss_feed, dump_json, datetime_filter, known_articles_ids)\
                        for rss_feed in self.rss_feeds]
            concurrent.futures.wait(futures)
            self.df_feeds = pd.concat([future.result() for future in futures])

            # rss_feeds_with_sentiment = self.nlp_pipe.get_sentiment(self.config['keywords'], self.rss_feeds.reset_index(drop=True))
